...
const DynamiceRtx = ({ user }) => {
  const { id } = useParams();
  const navigate = useNavigate();
  const CardRtx = user.find((i) => i.id === parseInt(id));
  return (
    <div className="w-full bg-neutral-500 min-h-screen  flex items-center justify-center p-6 relative">
      {/* Close Button */}
      <button
        onClick={() => navigate(-1)}
        className="absolute top-10 right-8 md:top-95 lg:top-28 lg:right-65   text-gray-700 text-3xl hover:text-gray-900 transition"
      >
      ...
      <div className="bg-white shadow-lg rounded-xl max-w-5xl w-full grid md:grid-cols-2 gap-6 overflow-hidden">
        ...
        <div className="p-6 flex flex-col justify-between">
          <div className="space-y-4">
            ...
            <p className="text-lg text-gray-600"> {CardRtx.Rrx}</p>
          </div>
