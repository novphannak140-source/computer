...
const DynamiceKeyborad = ({ user }) => {
  const { id } = useParams();
  const navigate = useNavigate();
  const CardKeyborad = user.find((i) => i.id === parseInt(id));
  return (
    <div className="w-full bg-neutral-500 min-h-screen  flex items-center justify-center p-6 relative">
      {/* Close Button */}
      <button
        onClick={() => navigate(-1)}
        className="absolute top-15 right-9 md:top-100 lg:top-35 lg:right-65   text-gray-700 text-3xl hover:text-gray-900 transition"
      >
      ...
      <div className="bg-white shadow-lg rounded-xl max-w-5xl w-full grid md:grid-cols-2 gap-6 overflow-hidden">
        ...
        <div className="flex items-center justify-center bg-gray-200">
          <img
            ...
            src={CardKeyborad.image}
            alt={CardKeyborad.brand}
          />
        </div>
...
export default DynamiceKeyborad;
