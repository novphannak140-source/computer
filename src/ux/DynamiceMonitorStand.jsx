...
const DynamiceMonitorStand = ({ user }) => {
  const { id } = useParams();
  const navigate = useNavigate();
  const CardMonitorStand = user.find((i) => i.id === parseInt(id));
  return (
    <div className="w-full bg-neutral-500 min-h-screen  flex items-center justify-center p-6 relative">
      {/* Close Button */}
      <button
        onClick={() => navigate(-1)}
        className="absolute top-15 right-9 md:top-100 lg:top-37 lg:right-65   text-gray-700 text-3xl hover:text-gray-900 transition"
      >
      ...
      <div className="bg-white shadow-lg rounded-xl max-w-5xl w-full grid md:grid-cols-2 gap-6 overflow-hidden">
        ...
        <div className="flex items-center justify-center bg-gray-200">
          <img
            ...
            src={CardMonitorStand.image}
            alt={CardMonitorStand.brand}
          />
        </div>
        ...
        <div className="p-6 flex flex-col justify-between">
          <div className="space-y-4">
            <h2 className="text-3xl font-bold text-gray-800">
              {CardMonitorStand.brand}
            </h2>
            <p className="text-xl text-gray-600">
              Price :{" "}
              <span className="font-semibold text-green-600">
                {CardMonitorStand.Price}
              </span>
            </p>
            <p className="text-lg text-gray-600">{CardMonitorStand.display}"</p>
          </div>
...
export default DynamiceMonitorStand;
